...
typedef struct ResourcePathClass
{
	uint8_t numberOfMonsters;
	uint8_t encounterChance; // Should be a number in the range [0-100]. This is the chance in any given minute that a monter will attack.
	uint8_t numberOfBackgroundImages;
	uint8_t monsterUnlockLevel; // When the base level of the location is greater than or equal to this, all monster types are available.
...
typedef struct ResourceFixedClass
{
	uint8_t monster; // A fixed location only has one monster, to be used if the arrival function is ShowBattleWindow
	int backgroundImage; // resource id
} ResourceFixedClass;

// The dungeon class interprets Location's length as per floor
typedef struct ResourceDungeonClass
{
	uint8_t numberOfFloors;
	uint8_t levelIncreaseRate; // For every x floors, add 1 to the baseLevel.
/*	PathClass *pathclass;
	FixedClass *fixedclass;*/
} ResourceDungeonClass;
